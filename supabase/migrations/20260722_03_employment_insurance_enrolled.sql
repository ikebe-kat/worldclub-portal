-- ═══════════════════════════════════════════════════════════════
-- worldclub-portal: 雇用保険 加入/非加入 のマスタ管理
--
-- 【背景】
--   wc_fn_calculate_monthly_payroll は雇用保険を全員一律
--     v_emp_ins := floor(v_gross::numeric * v_emp_insurance_rate)::int;
--   で課している。加入判定が無いため、WCパートで社保・雇保とも非加入の
--   6名（松浦・小池・加藤・増田・秋田・中嶋）にも雇保が引かれてしまう
--   （社保は既にマスタ social_insurance=0 で正しく非加入運用）。
--
-- 【修正】
--   1. wc_payroll_settings に加入フラグ列を新設
--        employment_insurance_enrolled boolean NOT NULL DEFAULT true
--      （既存行は自動で true=加入となる。新規社員も既定加入）
--   2. 上記6名を false に UPDATE（employee_code で対象特定）
--   3. wc_fn_calculate_monthly_payroll を修正：
--        IF COALESCE(v_setting.employment_insurance_enrolled, true) THEN
--          v_emp_ins := floor(v_gross::numeric * v_emp_insurance_rate)::int;
--        ELSE
--          v_emp_ins := 0;
--        END IF;
--      関数側は氏名/idを一切直書きせず、マスタのフラグだけを見る。
--
-- 【副作用】
--   非加入者は v_emp_ins=0 となり、その結果
--     v_taxable = v_gross - v_commute - v_shaho - v_child - v_emp_ins
--   の課税対象が雇保ぶんだけ増えるため、所得税(v_inc_tax)が数十円〜数百円
--   微増する可能性がある。所得税テーブル引きロジック自体は不変。会計上は
--   「雇保控除が外れたぶん課税対象が正しく戻る」挙動。
--
-- 【変更しない】
--   - social_insurance の値・扱いは不変
--   - 正社員・他の加入パート・田中(その他) の雇保額は既定true→従来通り
--   - shortfall/半休控除/有給集計などのロジックは不変
--   - 既存トリガー・KAT/明石共通関数・attendance_daily 実データは触らない
--   - 先月分の遡及計算は本migrationでは実行しない
--     （wc_payroll_monthly のUPDATEも一切しない）
--
-- 【適用手順（池邉さん）】
--   Supabase Dashboard → SQL Editor で本ファイル全体を貼り付けて実行、
--   または supabase db query --linked --file <path> でも可。
--   今月分の反映は給与管理タブで対象月を開いて「再計算」→ draft行のみ更新される。
--   confirmed 済み行は保護されるため、必要なら一旦戻して再計算 → 再確定。
-- ═══════════════════════════════════════════════════════════════

-- ── ① 加入フラグ列を追加（既定 true=加入） ──
ALTER TABLE public.wc_payroll_settings
  ADD COLUMN IF NOT EXISTS employment_insurance_enrolled boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN public.wc_payroll_settings.employment_insurance_enrolled IS
  '雇用保険加入フラグ。true=雇用保険を給与から控除する（既定）、false=控除しない（未加入者）。給与計算関数 wc_fn_calculate_monthly_payroll がこのフラグを参照して雇保額(v_emp_ins)を0にするかを判定する。social_insurance(社保)とは独立管理。';

-- ── ② 非加入6名を false に更新 ──
-- 対象: 松浦潤子(WC015)/小池眞加(WC007)/加藤知子(WC009)/増田彩華(WC006)/
--       秋田奈津季(WC012)/中嶋亜紀(WC013) — いずれも既に社保も非加入
UPDATE public.wc_payroll_settings s
   SET employment_insurance_enrolled = false,
       updated_at = now()
  FROM public.employees e
 WHERE e.id = s.employee_id
   AND s.company_id = 'c2d368f0-aa9b-4f70-b082-43ec07723d6c'::uuid
   AND e.employee_code IN ('WC006', 'WC007', 'WC009', 'WC012', 'WC013', 'WC015');

-- ── ③ 給与計算関数の再定義 ──
-- 20260722_01_shortfall_half_day.sql (現行世代) をベースに、雇用保険計算だけ
-- フラグ参照分岐に変更。他ロジックは一切変えない。
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      -- ★変更: 雇用保険は加入フラグを参照。false なら 0、true(既定)なら従来通り。
      --   関数側は氏名/idを一切直書きせず、マスタ列だけ見る。
      --   既存行は DEFAULT true なので既加入者の挙動は不変。
      IF COALESCE(v_setting.employment_insurance_enrolled, true) THEN
        v_emp_ins := floor(v_gross::numeric * v_emp_insurance_rate)::int;
      ELSE
        v_emp_ins := 0;
    ...
    INSERT INTO public.wc_payroll_monthly (
      ...
      jsonb_build_object(
        ...
        'employment_insurance_enrolled', COALESCE(v_setting.employment_insurance_enrolled, true),
        'late_early_absence_deduction', v_late_early_ded,
        'shortfall_min', v_shortfall_min,
        'expected_min',  v_expected_min,
        'half_leave_days', v_half_leave_days,
