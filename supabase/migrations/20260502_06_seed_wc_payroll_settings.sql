...
-- ── パート ──
-- scheduled_end_time / scheduled_minutes:
--   始業10:00、休憩1時間（松浦のみ40分）想定で算出。
--   17:00終業 = 6時間勤務 - 60分休憩 = 360分（6時間）
--   ※ scheduled_minutes は「所定労働時間（分）」= 有給1日の支給算出根拠
WITH wc AS (SELECT 'c2d368f0-aa9b-4f70-b082-43ec07723d6c'::uuid AS cid)
INSERT INTO public.wc_payroll_settings
  (company_id, employee_id, display_name, employment_type,
   ...
   social_insurance, commute_per_day, dependents)
SELECT
  wc.cid, e.id, v.name, 'パート',
  v.hw, v.hwe, v.endt::time, v.smin,
  v.bfix, v.pos, v.shaho, v.commute, 0
FROM wc
CROSS JOIN (VALUES
  -- emp_code, name,             hw,   hwe,  end,    smin, bfix, pos, shaho, commute
  ('WC005',  '小澤直美',         1150, 1250, '19:00', 480, NULL,    0, 23995, 120),
  ('WC006',  '増田彩華',         1300, 1400, '19:00', 480, NULL,    0,     0, 520),
  ('WC007',  '小池眞加',         1500, 1600, '19:00', 480, NULL,    0,     0, 240),
  ('WC008',  '島寄裕子',         1300, 1400, '17:00', 360, NULL,    0, 23880, 128),
  ('WC009',  '加藤知子',         1300, 1400, '17:00', 360, NULL,    0,     0, 160),
  ('WC010',  '南亜矢子',         1450, 1550, '19:00', 480, NULL, 10000, 36699,   0),
  ('WC011',  '新田真由美',       1300, 1400, '17:00', 360, NULL,    0, 23880,   0),
  ('WC012',  '秋田奈津季',       1150, 1250, '16:00', 300, NULL,    0,     0, 140),
  ('WC013',  '中嶋亜紀',         1150, 1250, '17:00', 360, NULL,    0,     0, 460),
  ('WC014',  '小寺慎一',         1150, 1250, '19:00', 480, NULL,    0, 28230, 900),
  ('WC015',  '松浦潤子',         1450, 1450, '15:40', 300,   40,    0,     0, 108),
  ('WC016',  '塚田真由美',       1250, 1350, '19:00', 480, NULL,    0, 28230, 480)
