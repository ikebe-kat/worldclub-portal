-- ═══════════════════════════════════════════════════════════════
-- worldclub-portal: 有給集計の半休対応
--
-- 【バグ】
--   wc_fn_calculate_monthly_payroll の有給日数集計が
--     COUNT(*) FILTER (WHERE reason LIKE '%有給（全日）%')
--   になっており、午前有給/午後有給が集計から漏れていた。
--   例: 小森達也 2026-06 → 実際 1.5日 (6/24午前有給 + 6/27全日) だが 1.0日で計上。
--
-- 【修正】
--   1. wc_payroll_monthly.paid_leave_days: integer → numeric(4,1) に型変更
--      （0.5日単位の格納を可能にする）
--   2. wc_fn_calculate_monthly_payroll の集計を
--        SUM(CASE
--          WHEN reason LIKE '%有給（全日）%' THEN 1.0
--          WHEN reason LIKE '%午前有給%' OR reason LIKE '%午後有給%' THEN 0.5
--          ELSE 0
--        END)
--      に統一。TS側 lib/leaveDays.ts countPaidLeaveDays() と同一ルール。
--   3. 有給金額計算(v_paid_leave_amt = 平日時給 × 所定分/60 × 日数) は既存のまま
--      → 半休は自動的に半額分だけ加算される。
--
-- 【触らないもの】
--   - paid_leave_grants (有給残高) には一切UPDATEしない
--   - wc_payroll_monthly の既存行にも直接UPDATEしない
--     → 適用後 PayrollSub の「再計算」ボタンで draft行のみ更新される
--   - 既存トリガー wc_fn_calc_attendance_daily / wc_fn_sync_over_under は変更なし
--     （全日休のときに actual_hours を NULL 化する挙動は正しいまま）
...
-- ── ① 列型を numeric(4,1) に変更（0.5日単位を保持できるように） ──
ALTER TABLE public.wc_payroll_monthly
  ALTER COLUMN paid_leave_days TYPE numeric(4,1) USING paid_leave_days::numeric(4,1);

-- ── ② 給与計算関数の再定義 ──
-- 現行 (20260716_01_payroll_leave_exclusion.sql) をベースに、有給COUNTだけ半休対応。
...
DECLARE
  ...
  v_weekend_min     int;
  v_overtime_min    int;
  v_paid_leave_days numeric(4,1);   -- ★変更: 半休(0.5)対応で numeric に
  v_absent_days     int;
  v_base int;
  v_fixed int;
  v_pos int;
  ...
  LOOP
    ...
    IF NOT v_is_excluded AND NOT v_setting.is_payroll_only AND v_setting.employee_id IS NOT NULL THEN
      ...
      SELECT
        ...
        -- ★変更: 全日=1.0 / 午前・午後=0.5 で合算。TS側 countPaidLeaveDays() と同一ルール。
        COALESCE(SUM(CASE
          WHEN reason LIKE '%有給（全日）%' THEN 1.0
          WHEN reason LIKE '%午前有給%' OR reason LIKE '%午後有給%' THEN 0.5
          ELSE 0
    ...
    IF v_is_excluded THEN
      ...
      v_commute := 0; v_late_early_ded := 0; v_shortfall_min := 0;
    ...
    ELSE
      ...
      IF v_setting.employment_type = 'パート' THEN
        ...
        -- 半休は v_paid_leave_days = 0.5 として乗る（金額も半額）
