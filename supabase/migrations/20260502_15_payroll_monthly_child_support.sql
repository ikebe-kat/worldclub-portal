-- ═══════════════════════════════════════════════════════════════
-- worldclub-portal: wc_payroll_monthly に child_support_allowance を追加
-- + wc_fn_calculate_monthly_payroll を再定義して支援金を集計に組み込む
-- ═══════════════════════════════════════════════════════════════

ALTER TABLE public.wc_payroll_monthly
  ADD COLUMN IF NOT EXISTS child_support_allowance integer NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.wc_payroll_monthly.child_support_allowance IS
  '子育て支援金（月額）。wc_payroll_settings から転記';

-- ── 計算関数を再定義（child_support_allowance を gross に加算） ──
